...
function HomePage() {
  ...
  return <Page>
    ...
    <section className="mt-5 grid gap-3 sm:gap-5 lg:grid-cols-[0.8fr_1.2fr]">
      ...
      <Surface className="sm:p-5 md:p-6"><div className="grid grid-cols-[minmax(0,1fr)_auto] items-center gap-3"><div className="min-w-0"><p className="text-sm text-muted-foreground">This Week</p><h2 className="mt-0.5 truncate font-display text-xl font-bold">{week.total ? "Your current pace" : "No activity yet"}</h2></div><BookOpen className="h-5 w-5 shrink-0 text-primary" /></div><div className="mt-4 grid grid-cols-3 gap-3 sm:mt-6"><Metric label="Accuracy" value={week.total ? `${week.accuracy}%` : "—"} /><Metric label="Questions" value={week.total} /><Metric label="Study time" value={formatDuration(week.studySeconds)} /></div><div className="mt-4 border-t border-border pt-4 sm:mt-5 sm:pt-5"><TrendChart compact label="Questions practiced this week" data={activity.map((day) => ({ label: day.date.slice(5), value: day.questions || null }))} /></div></Surface>
    </section>
  </Page>;
}